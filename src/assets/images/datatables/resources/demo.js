...
if ( window.$ ) {
	$(document).ready( function () {
		...
		$('div.tabs div.table').append(
			'<code className="multiline language-html">\t\t\t'+
			...
			$(document).on( 'init.dt', function ( e, settings ) {
				...
				var show = function ( str ) {
					...
					$('div.tabs div.ajax').append(
						$('<code className="multiline language-js"/>').text( str )
					);
					...
					$.ajax( {
						...
						success: function ( txt ) {
							...
							$('div.tabs div.php').append(
								'<code className="multiline language-php">'+txt+'</code>'
							);
		...
		$('ul.tabs').on( 'click', 'li', function () {
			$('ul.tabs li.active').removeclassName('active');
